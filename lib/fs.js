...
function overwriteFs () {
  ...
  overwrite('copyFileSync', (copyFileSync) => {
    return function (src, dest, mode) {
      ...
      const arr = splitPath(path.resolve(src))
      if (!arr[0] || arr[2] === '') return copyFileSync.apply(this, arguments)

      const fd = fs.openSync(arr[1], 'r')
      ...
      let stats
      try {
        stats = asar.statFile(arr[1], arr[2])
      } catch (err) {
        fs.closeSync(fd)
      ...
      if (stats.unpacked) {
        fs.closeSync(fd)
        return copyFileSync.call(this, path.join(arr[1] + '.unpacked', arr[2]), dest, mode)
      }
