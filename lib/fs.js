...
function overwriteFs () {
  ...
  overwrite('realpathSync', (realpathSync) => {
    return function (p, options) {
      if (process.noAsar) return realpathSync.apply(this, arguments)
      p = bufferToString(p)
      ...
      const result = path.join(realpathSync(asarPath), pathInAsar)
  ...
  overwrite('realpath', (realpath) => {
    return function (p, options, callback) {
      if (process.noAsar) return realpath.apply(this, arguments)
      p = bufferToString(p)
      let { isAsar, asarPath, pathInAsar } = splitPath(path.resolve(p))
      if (!isAsar || pathInAsar === '') return realpath.apply(this, arguments)
      if (typeof options === 'function') {
        callback = options
        options = undefined
      }
