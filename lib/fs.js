    ...
    return function (p, options, callback) {
      ...
      process.nextTick(() => {
        ...
        try {
          ...
          callback && callback(ENOENT('no such file or directory, asar lstatSync \'' + p + '\''))
          return
        }
        callback && callback(null, stat)
      })
    }
  })
    ...
    return function (p, options, callback) {
      ...
      process.nextTick(() => {
        ...
        try {
          ...
          callback && callback(ENOENT('no such file or directory, asar readdir \'' + p + '\''))
          return
        }
        if (node.files) {
          const names = Object.keys(node.files)
        ...
        callback && callback(ENOTDIR('not a directory, asar readdir \'' + p + '\''))
      })
    }
  })
    ...
    const _realpath = function (p, asarPath, pathInAsar, options, callback) {
      ...
      process.nextTick(() => {
        ...
        try {
          ...
          callback && callback(ENOENT('no such file or directory, asar realpath \'' + p + '\''))
          return
        }
        if (stat.link) pathInAsar = stat.link
        oldRealpath(asarPath, (err, real) => {
          ...
          if (options.encoding === 'buffer') {
            return callback && callback(null, Buffer.from(result))
          }
          return callback && callback(Buffer.from(result).toString(options.encoding))
        })
      })
    }
    ...
    return function (src, dest, mode, callback) {
      ...
      process.nextTick(() => {
        ...
        try {
          ...
          callback && callback(ENOENT('no such file or directory, open \'' + src + '\''))
          return
        }
        ...
        ws.once('error', (err) => { callback && callback(err) })
        ws.on('close', () => {
          callback && callback(null)
        })
        rs.once('error', (err) => { callback && callback(err) })
    ...
    return function (p, flags, mode, callback) {
      ...
      process.nextTick(() => {
        ...
        try {
          ...
          return callback && callback(ENOENT('no such file or directory, asar open \'' + p + '\''))
        }

        if (stats.files) {
          return callback && callback(EISDIR('illegal operation on a directory'))
        }
        ...
        open.apply(this, [asarPath, flags, mode, (err, fd) => {
          ...
          try {
            ...
            return callback && callback(ENOENT('no such file or directory, asar openSync \'' + p + '\''))
          }
          const wrappedFd = new FileDescriptor(fd, 8 + headerSize + parseInt(stats.offset, 10), 8 + headerSize + parseInt(stats.offset, 10) + stats.size)
          callback && callback(null, wrappedFd)
        }])
      })
    }
  })
  ...
  overwrite('read', (read) => {
    return function (fd, buffer, offset, length, position, callback) {
      if (fd instanceof FileDescriptor) {
        ...
        if (length > buffer.byteLength) {
          process.nextTick(() => {
            callback && callback(new RangeError(`The value of "length" is out of range. It must be <= ${buffer.byteLength}. Received ${length}`))
          })
          return
        }
        read.apply(this, [fd.fd, buffer, offset, length, position, (err, bytesRead) => {
          ...
          fd.pos = position + bytesRead
          callback && callback(null, bytesRead)
        }])
      } else {
